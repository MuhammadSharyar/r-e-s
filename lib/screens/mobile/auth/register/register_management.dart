...
class RegisterManagement extends StatelessWidget {
  RegisterManagement({Key? key}) : super(key: key);
  ...
  String managementSecurityKey = "";

  Future registerManagment() async {
    if (schoolEmailController.text == "" ||
        nameController.text == "" ||
      ...
      try {
        await FirebaseFirestore.instance
            ...
            .get()
            .then((value) =>
                managementSecurityKey = value.data()!['keyForManagement']);

        if (keyController.text.trim() != managementSecurityKey) {
          errorController.setErrorMessage('Your provided key is incorrect.');
        } else {
          loadingController.setLoading(true);
          errorController.setErrorMessage('');
          await AuthService().createManagementAccount(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text("Management Registration")),
      body: SafeArea(
